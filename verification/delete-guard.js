...
async function mk(label, withOrder) {
...
(async () => {
  ...
  try {
    ...
    // 2. CONTROL: same fixture through the unguarded copy reproduces the orphan
    r = await del(3232, A.id);
    ok(r.s === 200 && !(await exists(A.id)) && (await orderRow(A.order.id))?.auction_id === A.id, 'CONTROL (guard removed): delete succeeds and leaves the order orphaned - the old bug reproduced');
    ...
    // 4b. UPPERCASE id bypass (orders.auction_id is text, so the order check was case-sensitive)
    const E = await mk('upper_old', true); made.push(E);
    r = await del(3234, E.id.toUpperCase());
    ok(r.s === 200 && !(await exists(E.id)) && (await orderRow(E.order.id))?.auction_id === E.id, 'PINNED 3c6dc5a: DELETE with an UPPERCASE id deletes an auction that has an order and orphans it  <- bypass reproduced');
    ...
    const left = (await s.from('auctions').select('id').like('title', 'ZZTEST_delguard%')).data.length + (await s.from('orders').select('id').eq('buyer_username', 'zztest_delguard')).data.length;
